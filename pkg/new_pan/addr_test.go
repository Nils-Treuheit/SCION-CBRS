...
func TestUDPAddrIsValid(t *testing.T) {
	ia := new_pan.MustParseIA("1-ff00:0:0")
	iaWildcard := new_pan.MustParseIA("1-0")
	ip := netip.MustParseAddr("127.0.0.1")
	cases := []struct {
		addr    new_pan.UDPAddr
		isValid bool
	}{
		{new_pan.UDPAddr{}, false},
		{new_pan.UDPAddr{IA: ia}, false},
		{new_pan.UDPAddr{IP: ip}, false},
		{new_pan.UDPAddr{IA: ia, IP: ip}, true},
		{new_pan.UDPAddr{IA: iaWildcard, IP: ip}, false},
	}
	for _, c := range cases {
