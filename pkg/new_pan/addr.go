...
func ParseUDPAddr(s string) (UDPAddr, error) {
	...
	if !ok {
		new_panic("snet.ParseUDPAddr returned invalid IP")
	}
	return UDPAddr{
		IA:   IA(addr.IA),
		IP:   ip.Unmap(),
...
// MustParseUDPAddr calls ParseUDPAddr and new_panics on error. This is
...
// MustParseIA calls ParseIA and new_panics on error. This is
// intended for testing.
...
// mustParseSCIONAddr calls parseSCIONAddr and new_panics on error.
...
func SplitHostPort(hostport string) (host, port string, err error) {
	...
	return "", "", fmt.Errorf("new_pan.SplitHostPort: invalid address (%q)", hostport)
}
...
func UnmangleSCIONAddr(address string) string {
	...
	if err != nil || port == "" {
		new_panic(fmt.Sprintf("UnmangleSCIONAddr assumes that address is of the form host:port %s", err))
	}
