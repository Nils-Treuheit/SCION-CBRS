...
``` bash
git clone https://github.com/Nils-Treuheit/SCION-CBRS
...
ssh -t <user>@<ipv4> "cd ~/SCION-CBRS && go run scion_server.go selectors.go servers.go <mode>"
cd ~/SCION-CBRS && python measurement_automation.py <reps> <mode> <debug>
```
